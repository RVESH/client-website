...
function error(c, message = "Something went wrong", status = 500, details = null) {
  return c.json(
    {
      success: false,
      message,
...
async function updateUser(env, id, updates) {
  const db = getDatabase(env);
...
async function createOtp(env, otpData) {
  const db = getDatabase(env);
  const id = crypto.randomUUID();
  await db.prepare(`
        INSERT INTO otps (
...
async function findOtp(env, filter) {
  const db = getDatabase(env);
...
__name(findOtp, "findOtp");
async function deleteOtp(env, filter) {
  const db = getDatabase(env);
  await db.prepare(`
        DELETE FROM otps
        WHERE id = ?
...
async function deleteOtps(env, filter) {
  const db = getDatabase(env);
  await db.prepare(`
        DELETE FROM otps
        WHERE email = ?
...
// src/services/otp/.....ifMONGODBotp.service.js
...
async function generateAndStoreOtp(env, email, purpose) {
...
async function verifyOtpService(env, email, otp, purpose) {
...
__name(verifyOtpService, "verifyOtpService");
...
async function sendOtpEmail(env, email, otp) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Brevo Error: ${error2}`);
  }
...
// src/services/auth/......ifMONGODBregister.service.js
...
async function registerService(env, payload) {
  ...
  if (existingUser) {
    ...
    const { otp: otp2 } = await generateAndStoreOtp(
      env,
      normalizedEmail,
      "REGISTER"
    );
    console.log("Sending OTP");
    await sendOtpEmail(env, normalizedEmail, otp2);
...
var worker_default = {
  async fetch(request, env, ctx) {
    return app_default.fetch(request, env, ctx);
  }
};
...
// .wrangler/tmp/bundle-d4PHz8/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-d4PHz8/middleware-loader.entry.ts
